...
from app.infrastructure.db.migrator import Migrator
import re

RAW_TEXT = """module.exports = `
#---------- Список жанров fb2 ----------
0.1 Фантастика
...
def build_tree(genres):
    """
    Возвращает список с parent_idx
    """
    for g in genres:
        parts = g["idx"].split(".")
        if len(parts) == 1:
            ...
            g["parent_idx"] = ".".join(parts[:-1])

    return genres


def insert_sqlite(genres, router):
    with router.meta() as conn:
        cur = conn.cursor()

        # idx -> id
        idx_to_id = {}

        # сначала сортируем, чтобы родители шли раньше
        genres_sorted = sorted(genres, key=lambda x: [int(p) for p in x["idx"].split(".")])

        for g in genres_sorted:
            parent_id = idx_to_id.get(g["parent_idx"])

            cur.execute("""
                INSERT INTO genres (parent_id, name_ru, name_en)
                VALUES (?, ?, ?)
            """, (parent_id, g["name_ru"], g["name_en"]))

            idx_to_id[g["idx"]] = cur.lastrowid

if __name__ == "__main__":
    genres = parse_genres(RAW_TEXT)
    genres = build_tree(genres)
    router = DBRouter()
    Migrator(router).migrate_meta()

    insert_sqlite(genres, router)
