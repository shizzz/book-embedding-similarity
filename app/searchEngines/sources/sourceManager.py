    ...
    async def _download_archive(
        ...
        def _download():
            with ConnectionFactory.create(self.folder) as conn:
                progress_idx = None
                if self._ui:
                    progress_idx = self._ui.add_progress(
                        f"Downloading {archive_name}", "B"
                    )
                    total_size = conn.get_file_size(archive_name)
                    if self._ui and progress_idx is not None:
                        self._ui.update_total(total_size, progress_idx)

                def progress_callback(bytes_read: int):
                    if self._ui and progress_idx is not None:
                        self._ui.done(progress_idx, bytes_read)

                # теперь conn.download умеет вызывать callback
                conn.download(archive_name, tmp_path, progress_callback=progress_callback)
